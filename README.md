...
1. 관리자 페이지 접속 (/admin)
   ↓
2. "일일 코드 생성" 섹션에서 새 코드 생성
   예: 코드 "2024", 날짜 "2025-11-05", 비밀번호 입력
   → 코드가 비활성 상태로 생성됨
   ↓
3. "입장 코드별 현황" 섹션에서 생성된 코드 확인
   → [2024] 2025-11-05 | 0명 참가 | [비활성] [활성화] [상세보기] [삭제]
   ↓
4. 이벤트 시작 전 "활성화" 버튼 클릭
   → [2024] 2025-11-05 | 0명 참가 | [활성] [비활성화] [상세보기]
   ↓
5. 참가자들이 코드 "2024"로 입장 시작
   → [2024] 2025-11-05 | 15명 참가 | [활성] [비활성화] [상세보기]
   ↓
6. "상세보기" 클릭하여 실시간 참가자 및 팀 구성 확인
   ↓
7. 이벤트 종료 후 "비활성화" 버튼 클릭
   → 더 이상 해당 코드로 입장 불가
```
